...
import { setAuth, setThemeStore, setCartStore, setConfirm, resetTestState, resetApiMock, renderPage, authUser, sampleListing ,renderPageWithRoute } from '../../test-utils';

import VirtualTryOn from '../VirtualTryOn';

beforeEach(() => { resetTestState(); resetApiMock(api); setAuth(authUser()); setThemeStore(); setCartStore(); setConfirm(); });

describe('VirtualTryOn page', () => {
  ...
  test('renders without crashing', async () => {
    stubAll();
    renderPageWithRoute(<VirtualTryOn />, { route: '/test/abc', path: '/test/:id' });
