...
describe('Pagination', () => {
  ...
  test('next button is disabled on the last page', () => {
    render(<MemoryRouter future={routerFuture}><Pagination currentPage={5} totalPages={5} onPageChange={() => {}} /></MemoryRouter>);
    expect(screen.getByLabelText('Next page')).toBeDisabled();
  });
  test('shows the active page with aria-current', () => {
    render(<MemoryRouter future={routerFuture}><Pagination currentPage={3} totalPages={10} onPageChange={() => {}} /></MemoryRouter>);
  ...
  test('clicking a page calls onPageChange with that page', () => {
    ...
    render(<MemoryRouter future={routerFuture}><Pagination currentPage={1} totalPages={5} onPageChange={onPageChange} /></MemoryRouter>);
    fireEvent.click(screen.getByRole('button', { name: 'Page 2' }));
    expect(onPageChange).toHaveBeenCalledWith(2);
  });
  test('shows an ellipsis when pages exceed the window', () => {
    render(<MemoryRouter future={routerFuture}><Pagination currentPage={5} totalPages={20} onPageChange={() => {}} /></MemoryRouter>);
